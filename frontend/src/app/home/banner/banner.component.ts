...
import { Component, ElementRef, Renderer2, ViewChild, OnInit } from '@angular/core';
import { CombinedProductsService } from '../../services/combined.service';
import { Product } from './../../../interfaces/product.model';
...
export class BannerComponent implements OnInit {
  ...
  constructor(
    private renderer: Renderer2,
    private combinedProductsService: CombinedProductsService
  ) {}
  ...
  loadProducts(gender: string): void {
    this.combinedProductsService.getCombinedProducts(gender, this.currentPage, 3).subscribe(
      (response) => {
        this.carouselProducts = response.carouselProducts.map((product: { image: any; }) => ({
          ...product,
          image: `http://localhost:3000/images/${product.image}`
        }));
        this.products = response.paginatedProducts.map((product: { image: any; }) => ({
          ...product,
          image: `http://localhost:3000/images/${product.image}`
        }));
      },
